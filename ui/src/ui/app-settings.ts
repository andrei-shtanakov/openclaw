...
import type { OrchidApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as OrchidApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as OrchidApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as OrchidApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as OrchidApp);
    await loadConfig(host as unknown as OrchidApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as OrchidApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as OrchidApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as OrchidApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as OrchidApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as OrchidApp);
    await loadDevices(host as unknown as OrchidApp);
    await loadConfig(host as unknown as OrchidApp);
    await loadExecApprovals(host as unknown as OrchidApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as OrchidApp);
    await loadConfig(host as unknown as OrchidApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as OrchidApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as OrchidApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__ORCHID_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as OrchidApp;
...
async function loadOverviewLogs(host: OrchidApp) {
...
function buildAttentionItems(host: OrchidApp) {
  ...
  if (auth?.scopes && !auth.scopes.includes("operator.read")) {
    items.push({
      ...
      href: "https://docs.orchid.ai/web/dashboard",
      external: true,
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OrchidApp, true),
    loadConfigSchema(host as unknown as OrchidApp),
    loadConfig(host as unknown as OrchidApp),
  ]);
}

export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OrchidApp, false),
    loadCronStatus(host as unknown as OrchidApp),
    loadCronJobs(host as unknown as OrchidApp),
  ]);
}
