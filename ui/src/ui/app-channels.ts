...
export async function handleWhatsAppStart(host: OrchidApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: OrchidApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: OrchidApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: OrchidApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: OrchidApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: OrchidApp): string {
...
function resolveGatewayHttpAuthHeader(host: OrchidApp): string | null {
...
function buildGatewayHttpHeaders(host: OrchidApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: OrchidApp) {
...
export function handleNostrProfileToggleAdvanced(host: OrchidApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: OrchidApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: OrchidApp) {
  const state = host.nostrProfileFormState;
