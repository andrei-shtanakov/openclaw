...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/orchid/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/orchid");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/orchid/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ORCHID_CONTROL_UI_BASE_PATH__ = "/orchid";
    const app = mountApp("/orchid/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/orchid");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/orchid/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("orchid.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
