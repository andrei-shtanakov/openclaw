...
This document describes how the Orchid ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`orchid acp` exposes an ACP agent over stdio and forwards prompts to a running
Orchid Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `orchid acp` and speaks ACP messages over stdio.
...
`orchid acp` resolves the Gateway URL and auth from CLI flags or config:
