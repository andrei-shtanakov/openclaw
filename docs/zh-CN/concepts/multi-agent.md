...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.orchid/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.orchid/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.orchid`（或 `ORCHID_STATE_DIR`）
- 工作区：`~/.orchid/workspace`（或 `~/.orchid/workspace-<agentId>`）
- 智能体目录：`~/.orchid/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.orchid/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，Orchid 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.orchid/workspace`（或当设置了 `ORCHID_PROFILE` 时为 `~/.orchid/workspace-<profile>`）。
- 状态默认为 `~/.orchid/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.orchid/credentials/whatsapp/personal
