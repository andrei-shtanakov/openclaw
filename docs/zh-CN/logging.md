...
你可以在 `~/.orchid/orchid.json` 中覆盖此设置：
...
所有日志配置都在 `~/.orchid/orchid.json` 的 `logging` 下。
...
- Orchid 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `orchid plugins enable diagnostics-otel` 启用插件。
...
- `orchid.tokens`（计数器，属性：`orchid.token`、`orchid.channel`、`orchid.provider`、`orchid.model`）
- `orchid.cost.usd`（计数器，属性：`orchid.channel`、`orchid.provider`、`orchid.model`）
- `orchid.run.duration_ms`（直方图，属性：`orchid.channel`、`orchid.provider`、`orchid.model`）
- `orchid.context.tokens`（直方图，属性：`orchid.context`、`orchid.channel`、`orchid.provider`、`orchid.model`）

消息流：

- `orchid.webhook.received`（计数器，属性：`orchid.channel`、`orchid.webhook`）
- `orchid.webhook.error`（计数器，属性：`orchid.channel`、`orchid.webhook`）
- `orchid.webhook.duration_ms`（直方图，属性：`orchid.channel`、`orchid.webhook`）
- `orchid.message.queued`（计数器，属性：`orchid.channel`、`orchid.source`）
- `orchid.message.processed`（计数器，属性：`orchid.channel`、`orchid.outcome`）
- `orchid.message.duration_ms`（直方图，属性：`orchid.channel`、`orchid.outcome`）

队列 + 会话：

- `orchid.queue.lane.enqueue`（计数器，属性：`orchid.lane`）
- `orchid.queue.lane.dequeue`（计数器，属性：`orchid.lane`）
- `orchid.queue.depth`（直方图，属性：`orchid.lane` 或 `orchid.channel=heartbeat`）
- `orchid.queue.wait_ms`（直方图，属性：`orchid.lane`）
- `orchid.session.state`（计数器，属性：`orchid.state`、`orchid.reason`）
- `orchid.session.stuck`（计数器，属性：`orchid.state`）
- `orchid.session.stuck_age_ms`（直方图，属性：`orchid.state`）
- `orchid.run.attempt`（计数器，属性：`orchid.attempt`）
...
- `orchid.webhook.error`
  - `orchid.channel`、`orchid.webhook`、`orchid.chatId`、`orchid.error`
- `orchid.message.processed`
  - `orchid.channel`、`orchid.outcome`、`orchid.chatId`、`orchid.messageId`、`orchid.sessionKey`、`orchid.sessionId`、`orchid.reason`
- `orchid.session.stuck`
  - `orchid.state`、`orchid.ageMs`、`orchid.queueDepth`、`orchid.sessionKey`、`orchid.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `orchid doctor`。
