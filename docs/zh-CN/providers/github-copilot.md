...
read_when:
  ...
  - 你需要了解 `orchid models auth login-github-copilot` 流程
summary: 使用设备流从 Orchid 登录 GitHub Copilot
...
GitHub Copilot 是 GitHub 的 AI 编程助手。它为你的 GitHub 账户和订阅计划提供 Copilot 模型的访问权限。Orchid 可以将 Copilot 用作模型提供商。

使用原生设备登录流程获取 GitHub 令牌，然后在 Orchid 运行时将其兑换为 Copilot API 令牌。登录命令运行 GitHub 设备流程，保存认证配置文件，并更新你的配置以使用该配置文件。
...
- 登录会将 GitHub 令牌存储在认证配置文件中，并在 Orchid 运行时将其兑换为 Copilot API 令牌。
