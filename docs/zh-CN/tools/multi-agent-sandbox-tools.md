...
有关调试"为什么这被阻止了？"，请参见[沙箱 vs 工具策略 vs 提权](/gateway/sandbox-vs-tool-policy-vs-elevated) 和 `orchid sandbox explain`。
...
- `group:nodes`：`nodes`
- `group:orchid`：所有内置 Orchid 工具（不包括提供商插件）
...
旧版 `agent.*` 配置由 `orchid doctor` 迁移；今后请优先使用 `agents.defaults` + `agents.list`。
