...
- `~/Library/Application Support/Orchid/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `orchid-canvas://<session>/<path>`

示例：

- `orchid-canvas://main/` → `<canvasRoot>/main/index.html`
- `orchid-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `orchid-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
orchid nodes canvas present --node <id>
orchid nodes canvas navigate --node <id> --url "/"
orchid nodes canvas eval --node <id> --js "document.title"
orchid nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__orchid__/a2ui/
```
...
orchid nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
orchid nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "orchid://agent?message=Review%20this%20design";
```
