...
read_when:
  ...
  - Wiring external systems into Orchid
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Orchid config directory (typically `~/.orchid/hooks/transforms`).
...
- `orchid webhooks gmail setup` writes `hooks.gmail` config for `orchid webhooks gmail run`.
