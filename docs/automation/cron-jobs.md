...
- Jobs persist under `~/.orchid/cron/` so restarts don’t lose schedules.
...
orchid cron list
orchid cron run <job-id>
orchid cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.orchid/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `orchid cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Orchid applies a
...
If `delivery` is omitted for isolated jobs, Orchid defaults to `announce`.
...
- Job store: `~/.orchid/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.orchid/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
orchid cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
orchid cron edit <jobId> --agent ops
orchid cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
orchid cron run <jobId>
orchid cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
orchid cron edit <jobId> \
...
```bash
orchid system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`orchid system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `ORCHID_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Orchid applies exponential retry backoff for recurring jobs after consecutive errors:
