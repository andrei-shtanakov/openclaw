---
summary: "Use Venice AI privacy-focused models in Orchid"
read_when:
  - You want privacy-focused inference in Orchid
...
```bash
orchid onboard --auth-choice venice-api-key
```
...
```bash
orchid chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Orchid shows all available Venice models. Pick based on your needs:
...
```bash
orchid models set venice/claude-opus-45
orchid models set venice/llama-3.3-70b
```

List all available models:

```bash
orchid models list | grep venice
```

## Configure via `orchid configure`

1. Run `orchid configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Orchid automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
orchid chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
orchid chat --model venice/claude-opus-45

# Use uncensored model
orchid chat --model venice/venice-uncensored

# Use vision model with image
orchid chat --model venice/qwen3-vl-235b-a22b

# Use coding model
orchid chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `orchid models list` to see currently available models. Some models may be temporarily offline.
