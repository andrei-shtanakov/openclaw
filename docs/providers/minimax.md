---
summary: "Use MiniMax M2.1 in Orchid"
read_when:
  - You want MiniMax models in Orchid
...
```bash
orchid plugins enable minimax-portal-auth  # skip if already loaded.
orchid gateway restart  # restart if gateway is already running
orchid onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/orchid/orchid/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `orchid.json`:
...
- Use `orchid models list` and `orchid models set minimax/MiniMax-M2.1` to switch.
...
- Running `orchid configure` and selecting **MiniMax M2.1**, or
