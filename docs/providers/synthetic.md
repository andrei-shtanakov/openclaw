---
summary: "Use Synthetic's Anthropic-compatible API in Orchid"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Orchid registers it as the
...
Note: Orchid's Anthropic client appends `/v1` to the base URL, so use
