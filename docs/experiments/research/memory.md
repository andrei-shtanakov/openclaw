...
read_when:
  - Designing workspace memory (~/.orchid/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Orchid integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.orchid/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.orchid/workspace` as canonical human-readable memory.
...
```
~/.orchid/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Orchid**, but keep a separable core library.

### Why integrate into Orchid?

- Orchid already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `orchid memory recall "…" --k 25 --since 30d`
  - `orchid memory reflect --since 7d`
...
Pragmatic take for `~/.orchid/workspace`:
