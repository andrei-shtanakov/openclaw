---
summary: "How Orchid builds prompt context and reports token usage + costs"
read_when:
...
Orchid tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Orchid assembles its own system prompt on every run. It includes:
...
For images, Orchid downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Orchid session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `orchid status --usage` and `orchid channels list` show
...
`cacheWrite`. If pricing is missing, Orchid shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Orchid can
...
Anthropic's 1M context window is currently beta-gated. Orchid can inject the
