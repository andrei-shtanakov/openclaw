...
Orchid integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Orchid owns lifecycle when `channels.signal.autoStart=true`.
...
- Orchid spawns `imsg rpc` as a child process (legacy iMessage integration).
