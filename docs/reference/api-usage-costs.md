...
Orchid features that can generate provider usage or paid API calls.
...
- `orchid status --usage` and `orchid channels list` show provider **usage windows**
...
Orchid can pick up credentials from:
...
`orchid models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
