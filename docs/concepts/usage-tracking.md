...
- `/usage cost` in chats: local cost summary aggregated from Orchid session logs.
- CLI: `orchid status --usage` prints a full per-provider breakdown.
- CLI: `orchid channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
