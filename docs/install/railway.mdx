...
Deploy Orchid on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/orchid-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/orchid` — Control UI

## What you get

- Hosted Orchid Gateway + Control UI
...
- `ORCHID_STATE_DIR=/data/.orchid` (recommended)
- `ORCHID_WORKSPACE_DIR=/data/workspace` (recommended)
- `ORCHID_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Orchid state + workspace so you can migrate to another host without losing config or memory.
