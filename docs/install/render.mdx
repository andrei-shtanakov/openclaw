...
Deploy Orchid on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/orchid/orchid)
...
repository configures everything needed to run Orchid:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: ORCHID_STATE_DIR
        value: /data/.orchid
      - key: ORCHID_WORKSPACE_DIR
        value: /data/workspace
      - key: ORCHID_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/orchid`.
...
If you use the original Orchid repository, Render will not auto-deploy your Orchid. To update it, run a manual Blueprint sync from the dashboard.
...
For Orchid, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Orchid host.
