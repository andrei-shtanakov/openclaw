---
summary: "Chrome extension: let Orchid drive your existing Chrome tab"
read_when:
...
The Orchid Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate orchid-managed Chrome profile.
...
Orchid then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Orchid release (npm package) as static files. There is no separate “build” step.

After upgrading Orchid:

- Re-run `orchid browser extension install` to refresh the installed files under your Orchid state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Orchid ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `ORCHID_GATEWAY_TOKEN`)
...
- Open the tab you want Orchid to control.
...
- `ON`: attached; Orchid can drive that tab.
...
Debugging: `orchid sandbox explain`
...
`orchid browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `orchid browser extension install` first to copy the extension to a stable location under your Orchid state directory.
...
- **This is not isolated** like the dedicated orchid-managed profile.
