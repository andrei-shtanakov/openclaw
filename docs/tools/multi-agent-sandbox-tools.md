...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `orchid sandbox explain`.
...
Legacy `agent.*` configs are migrated by `orchid doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=orchid-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${ORCHID_STATE_DIR:-$HOME/.orchid}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
