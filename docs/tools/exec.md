...
- `gateway`/`node` approvals are controlled by `~/.orchid/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Orchid prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`orchid security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `orchid doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
