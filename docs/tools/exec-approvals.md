...
- **gateway host** → `orchid` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.orchid/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.orchid/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `orchid approvals allowlist ...`).
- `orchid security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `orchid doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.orchid/exec-approvals.json` directly.

CLI: `orchid approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
