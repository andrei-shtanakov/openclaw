---
summary: "Agent tool surface for Orchid (browser, canvas, nodes, message, cron) replacing legacy `orchid-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `orchid-*` skills
title: "Tools"
---

# Tools (Orchid)

Orchid exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `orchid-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `orchid.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Orchid logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:orchid`: all built-in Orchid tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`orchid node run`).
...
Orchid tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `orchid configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Orchid-managed browser.
...
- Quick smoke: `orchid nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `orchid gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Orchid clamps `tools.sessions.visibility` to `tree`.
