---
summary: "Orchid on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Orchid on a Raspberry Pi
  - Running Orchid on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Orchid on Raspberry Pi

## Goal

Run a persistent, always-on Orchid Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Orchid features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled orchid
...
```bash
# Check logs
journalctl -u orchid --no-pager -n 100

# Common fix: rebuild
cd ~/orchid  # if using hackable install
npm run build
sudo systemctl restart orchid
```
