...
service via the external `orchid` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`ORCHID_PROFILE`; legacy `com.orchid.*` is supported).
