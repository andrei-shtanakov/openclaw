...
## Quick check: `orchid security audit`
...
```bash
orchid security audit
orchid security audit --deep
orchid security audit --fix
orchid security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

Orchid is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
Orchid assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.orchid`, including `orchid.json`), treat them as a trusted operator.
...
If you run `--deep`, Orchid also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.orchid/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.orchid/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.orchid/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.orchid/credentials/oauth.json`
...
| -------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full Orchid state                                 | filesystem perms on `~/.orchid`                                                                   | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                          | filesystem perms on `~/.orchid/orchid.json`                                                       | yes      |
...
`orchid security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`orchid security audit` includes `config.insecure_or_dangerous_flags` when any
...
gateway:
  ...
  auth:
    mode: password
    password: ${ORCHID_GATEWAY_PASSWORD}
```
...
Orchid stores session transcripts on disk under `~/.orchid/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.orchid` (see the audit section below). If you need
...
Orchid can refresh the skills list mid-session:
...
- If you install plugins from npm (`orchid plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.orchid/extensions/<pluginId>/` (or `$ORCHID_STATE_DIR/extensions/<pluginId>/`).
  - Orchid uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
orchid pairing list <channel>
orchid pairing approve <channel> <code>
```
...
By default, Orchid routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Orchid has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.orchid/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.orchid or your logs.”

## Unsafe external content bypass flags

Orchid includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.orchid/orchid.json`: `600` (user read/write only)
- `~/.orchid`: `700` (user only)

`orchid doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `ORCHID_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__orchid__/canvas/` and `/__orchid__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_orchid-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `ORCHID_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `orchid doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `ORCHID_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `ORCHID_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Orchid
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Orchid verifies the identity by resolving the
...
- Orchid will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.orchid/` (or `$ORCHID_STATE_DIR/`) may contain secrets or private data:

- `orchid.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `orchid status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@orchid", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.orchid/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `orchid` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Orchid clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Orchid limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `orchid gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `ORCHID_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/orchid/orchid-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.orchid/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `orchid security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Orchid version
...
Found a vulnerability in Orchid? Please report responsibly:

1. Email: [security@orchid.ai](mailto:security@orchid.ai)
