...
read_when:
  - Running Orchid behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Orchid
...
- You run Orchid behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Orchid checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Orchid extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://orchid.example.com
    to: http://orchid-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`orchid security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Orchid config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `orchid security audit` and review findings
