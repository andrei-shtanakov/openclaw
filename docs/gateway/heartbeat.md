...
- During heartbeat runs, Orchid treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `orchid sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Orchid skips the heartbeat run entirely (no model call).
...
```bash
orchid system event --text "Check for urgent follow-ups" --mode now
```
