...
description: "Complete field-by-field reference for ~/.orchid/orchid.json"
---

# Configuration Reference

Every field available in `~/.orchid/orchid.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Orchid uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `orchid doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["orchid-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `orchid plugins install @orchid/mattermost`.
...
Orchid spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@orchid"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `orchid.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Orchid auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Orchid assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "orchid-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.orchid/sandboxes`
...
noVNC observer access uses VNC auth by default and Orchid emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `orchid-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.orchid/workspace",
        agentDir: "~/.orchid/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@orchid"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:orchid`     | All built-in tools (excludes provider plugins)                                           |
...
Orchid uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.orchid/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `ORCHID_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `orchid onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `orchid onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `orchid onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `orchid onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `orchid onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `orchid onboard --auth-choice minimax-api`.
...
- Loaded from `~/.orchid/extensions`, `<workspace>/.orchid/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or ORCHID_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `ORCHID_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.orchid-dev` + port `19001`), `--profile <name>` (uses `~/.orchid-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.orchid/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-orchid-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `ORCHID_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or ORCHID_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__orchid__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__orchid__/a2ui/`
...
- Also serves A2UI at `/__orchid__/a2ui/`.
...
- Hostname defaults to `orchid`. Override with `ORCHID_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.orchid/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `orchid dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.orchid/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${ORCHID_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.orchid/credentials/oauth.json`.
...
- Default log file: `/tmp/orchid/orchid-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `orchid doctor --fix` can strip unknown keys).
