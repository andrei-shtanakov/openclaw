...
read_when:
  - Setting up Twitch chat integration for Orchid
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Orchid connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
orchid plugins install @orchid/twitch
```

Local checkout (when running from a git repo):

```bash
orchid plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `ORCHID_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "orchid", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ORCHID_TWITCH_ACCESS_TOKEN env var)
...
```bash
ORCHID_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
