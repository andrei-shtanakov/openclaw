...
```bash
orchid plugins install @orchid/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
orchid plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Orchid" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `orchid pairing list nextcloud-talk`
  - `orchid pairing approve nextcloud-talk <CODE>`
