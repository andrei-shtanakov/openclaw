---
summary: "Mattermost bot setup and Orchid config"
read_when:
...
```bash
orchid plugins install @orchid/mattermost
```

Local checkout (when running from a git repo):

```bash
orchid plugins install ./extensions/mattermost
```
...
4. Configure Orchid and start the gateway.
...
- Approve via:
  - `orchid pairing list mattermost`
  - `orchid pairing approve mattermost <CODE>`
...
Use these target formats with `orchid message send` or cron/webhooks:
