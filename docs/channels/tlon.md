...
Tlon is a decentralized messenger built on Urbit. Orchid connects to your Urbit ship and can
...
```bash
orchid plugins install @orchid/tlon
```

Local checkout (when running from a git repo):

```bash
orchid plugins install ./extensions/tlon
```
...
By default, Orchid blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `orchid message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Orchid replies in-thread.
