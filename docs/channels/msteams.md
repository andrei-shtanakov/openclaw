...
```bash
orchid plugins install @orchid/msteams
```

Local checkout (when running from a git repo):

```bash
orchid plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Orchid will offer the local install path automatically.
...
3. Configure Orchid with those credentials.
...
- Talk to Orchid via Teams DMs, group chats, or channels.
...
- On startup, Orchid resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.orchid/orchid.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Orchid, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                  |
   | ------------------ | ------------------------------------------------------ |
   | **Bot handle**     | Your bot name, e.g., `orchid-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `orchid plugins install @orchid/msteams`
   - From a local checkout: `orchid plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Orchid in Teams", "full": "Orchid in Teams" },
...
Orchid handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Orchid only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/OrchidShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Orchid sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `orchid message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.orchid/msteams-polls.json`.
...
```bash
orchid message send --channel msteams \
...
orchid message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
orchid message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
orchid message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
orchid message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
