...
Matrix is an open, decentralized messaging protocol. Orchid connects as a Matrix **user**
...
```bash
orchid plugins install @orchid/matrix
```

Local checkout (when running from a git repo):

```bash
orchid plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `orchid plugins install @orchid/matrix`
   - From a local checkout: `orchid plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Orchid calls the same
     login endpoint, stores the access token in `~/.orchid/credentials/matrix/credentials.json`,
...
- On first connection, Orchid requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Orchid logs a warning.
...
`~/.orchid/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `orchid pairing list matrix`
  - `orchid pairing approve matrix <CODE>`
...
- On startup, Orchid resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
