...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Orchid to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
orchid plugins install @orchid/feishu
```

Local checkout (when running from a git repo):

```bash
orchid plugins install ./extensions/feishu
```
...
If you just installed Orchid, run the wizard:
...
2. Configuring app credentials in Orchid
...
1. You already ran `orchid channels add` for Feishu
2. The gateway is running (`orchid gateway status`)
...
```bash
orchid pairing approve feishu <CODE>
```
...
2. Run `orchid logs --follow` and look for `chat_id`
...
2. Run `orchid logs --follow` and look for `open_id`
...
| Command                  | Description                   |
| ------------------------ | ----------------------------- |
| `orchid gateway status`  | Show gateway status           |
| `orchid gateway install` | Install/start gateway service |
| `orchid gateway stop`    | Stop gateway service          |
| `orchid gateway restart` | Restart gateway service       |
| `orchid logs --follow`   | Tail gateway logs             |
...
4. Check logs: `orchid logs --follow`
...
5. Ensure the gateway is running: `orchid gateway status`
6. Check logs: `orchid logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.orchid/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.orchid/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
