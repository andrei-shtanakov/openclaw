...
read_when:
  - You want Orchid to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Orchid to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`orchid onboard`) and `orchid channels add` list optional channel plugins.
...
```bash
orchid plugins install @orchid/nostr
```

Use a local checkout (dev workflows):

```bash
orchid plugins install --link <path-to-orchid>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "orchid@example.com",
        "lud16": "orchid@example.com"
      }
    }
  }
}
