...
```
    ...
    Telegram does **not** use `orchid channels login telegram`; configure token in config/env, then start gateway.
...
orchid pairing approve telegram <CODE>
```
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `orchid doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
...
```
    ...
    - or read `chat.id` from `orchid logs --follow`
...
- DM messages can carry `message_thread_id`; Orchid routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Orchid can stream partial replies by sending a temporary Telegram message and editing it as text arrives.
    ...
    For text-only replies, Orchid keeps the same preview message and performs a final edit in place (no second message).

    For complex replies (for example media payloads), Orchid falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, Orchid skips the preview stream to avoid double-streaming.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Orchid retries as plain text.
...
```
    ...
    - `~/.orchid/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Orchid enqueues system events like:
...
```bash
orchid message send --channel telegram --target 123456789 --message "hi"
orchid message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `orchid channels status` warns when config expects unmentioned group messages.
    - `orchid channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `orchid logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, Orchid now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `ORCHID_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `ORCHID_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `ORCHID_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `open` requires `"*"`. `orchid doctor --fix` can resolve legacy `@username` entries to IDs.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (default: allowlist).
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `orchid doctor --fix` can resolve legacy `@username` entries to IDs.
