---
summary: "CLI reference for `orchid security` (audit and fix common security footguns)"
read_when:
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `orchid.browserConfigEpoch`) and recommends `orchid sandbox recreate --browser --all`.
...
```bash
orchid security audit --json | jq '.summary'
orchid security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
orchid security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
