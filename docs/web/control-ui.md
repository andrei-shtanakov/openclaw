...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/orchid`)
...
If the page fails to load, start the Gateway first: `orchid gateway`.
...
you revoke it with `orchid devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.orchid/orchid.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Orchid
...
Orchid **blocks** Control UI connections without device identity.
