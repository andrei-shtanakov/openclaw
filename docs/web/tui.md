...
```bash
orchid tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `orchid tui --deliver`
...
- Check the Gateway logs: `orchid logs --follow`.
- Confirm the agent can run: `orchid status` and `orchid models status`.
...
- No agents in picker: check `orchid agents list` and your routing config.
