...
- Publish as `orchid/plugin-sdk` (or export from core under `orchid/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OrchidPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OrchidConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OrchidConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OrchidConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OrchidConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OrchidConfig): string;
  };
};
```
...
- Introduce `orchid/plugin-sdk`.
- Add `api.runtime` to `OrchidPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `orchidRuntime: ">=2026.2.0"`).
