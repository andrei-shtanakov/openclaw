...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Orchid
...
Running Orchid via Node with `tsx` fails at startup with:

```
[orchid] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `orchid status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch orchid.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node orchid.mjs status` works on Node 25.
