/**
 * Twitch channel plugin for Orchid.
 *
...
import { buildChannelConfigSchema } from "orchid/plugin-sdk";
 ...
 * for Orchid. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OrchidConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OrchidConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OrchidConfig): boolean => {
