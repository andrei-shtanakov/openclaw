 ...
 */
function setTwitchAccount(cfg: OrchidConfig, account: Partial<TwitchAccountConfig>): OrchidConfig {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: ORCHID_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: OrchidConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var ORCHID_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.ORCHID_TWITCH_ACCESS_TOKEN?.trim();
