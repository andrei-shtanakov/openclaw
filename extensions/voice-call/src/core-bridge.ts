...
function resolveOrchidRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.ORCHID_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set ORCHID_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOrchidRoot(), "dist", "extensionAPI.js");
