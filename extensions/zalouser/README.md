...
Orchid extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
orchid channels login --channel zalouser
...
```bash
orchid message send --channel zalouser --target <threadId> --message "Hello from Orchid!"
```
...
```bash
orchid channels login --channel zalouser              # Login via QR
orchid channels login --channel zalouser --account work
orchid channels status --probe
orchid channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
orchid directory self --channel zalouser
orchid directory peers list --channel zalouser --query "name"
orchid directory groups list --channel zalouser --query "work"
orchid directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
orchid message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
orchid message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `orchid channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
orchid channels login --channel zalouser --account work
orchid message send --channel zalouser --account work --target <id> --message "Hello"
