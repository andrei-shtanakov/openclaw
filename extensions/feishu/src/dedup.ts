...
import { createDedupeCache, createPersistentDedupe } from "orchid/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.ORCHID_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["orchid-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".orchid");
}
