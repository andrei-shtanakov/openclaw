...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as OrchidConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as OrchidConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as OrchidConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as OrchidConfig);
