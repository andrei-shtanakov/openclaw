...
import type { OrchidPluginApi, OrchidPluginToolContext } from "../../../src/plugins/types.js";
...
function fakeApi(overrides: Partial<OrchidPluginApi> = {}): OrchidPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<OrchidPluginToolContext> = {}): OrchidPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: OrchidPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
