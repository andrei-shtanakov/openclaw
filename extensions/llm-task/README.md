...
`orchid.invoke --each`) without adding new Orchid code per workflow.
...
This extension depends on Orchid internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Orchid extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.orchid/extensions` as a standalone plugin directory.
