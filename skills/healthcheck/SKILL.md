---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Orchid deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Orchid cron scheduling for periodic checks, or version status checks on a machine running Orchid (laptop, workstation, Pi, VPS).
---

# Orchid Host Hardening

## Overview

Assess and harden the host running Orchid, then align it to a user-defined risk tolerance without breaking access. Use Orchid security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Orchid changes the host firewall, SSH, or OS updates; it does not.
...
5. Orchid gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Orchid can access sensitive data.
...
### 2) Run Orchid security audits (read-only)

As part of the default read-only checks, run `orchid security audit --deep`. Only offer alternatives if the user requests them:

1. `orchid security audit` (faster, non-probing)
2. `orchid security audit --json` (structured output)

Offer to apply Orchid safe defaults (numbered):

1. `orchid security audit --fix`

Be explicit that `--fix` only tightens Orchid defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Orchid version/update status (read-only)

As part of the default read-only checks, run `orchid update status`.
...
- Credential hygiene notes (location of Orchid creds, prefer disk encryption)
...
After Orchid install or first hardening pass, run at least one baseline audit and version check:

- `orchid security audit`
- `orchid security audit --deep`
- `orchid update status`

Ongoing monitoring is recommended. Use the Orchid cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `orchid cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `orchid update status`
...
Before creating, `orchid cron list` and match on exact `name`. If found, `orchid cron edit <id> ...`.
If not found, `orchid cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `orchid update status` (preferred for source checkouts and channels)
2. `npm view orchid version` (published npm version)

## Orchid command accuracy

Use only supported commands and flags:

- `orchid security audit [--deep] [--fix] [--json]`
- `orchid status` / `orchid status --deep`
- `orchid health --json`
- `orchid update status`
- `orchid cron add|list|runs|run`

Do not invent CLI flags or imply Orchid enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Orchid compaction:
