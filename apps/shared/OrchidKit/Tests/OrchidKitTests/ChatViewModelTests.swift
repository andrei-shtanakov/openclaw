...
private final class TestChatTransport: @unchecked Sendable, OrchidChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OrchidChatHistoryPayload]
    private let sessionsResponses: [OrchidChatSessionsListResponse]

    private let stream: AsyncStream<OrchidChatTransportEvent>
    private let continuation: AsyncStream<OrchidChatTransportEvent>.Continuation

    init(
        historyResponses: [OrchidChatHistoryPayload],
        sessionsResponses: [OrchidChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<OrchidChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OrchidChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OrchidChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [OrchidChatAttachmentPayload]) async throws -> OrchidChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return OrchidChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OrchidChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OrchidChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OrchidChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = OrchidChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = OrchidChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { OrchidChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = OrchidChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = OrchidChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = OrchidChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { OrchidChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
