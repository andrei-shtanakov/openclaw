...
import "@a2ui/lit/ui";
import { themeContext } from "@orchid/a2ui-theme-context";
...
class OrchidA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--orchid-a2ui-inset-top, 0px)
        var(--orchid-a2ui-inset-right, 0px)
        var(--orchid-a2ui-inset-bottom, 0px)
        var(--orchid-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--orchid-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--orchid-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--orchid-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--orchid-a2ui-empty-top, var(--orchid-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.orchidA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["orchid:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__orchidLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.orchidCanvasA2UIAction ??
      globalThis.orchidCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.orchidCanvasA2UIAction) {
...
if (!customElements.get("orchid-a2ui-host")) {
  customElements.define("orchid-a2ui-host", OrchidA2UIHost);
}
