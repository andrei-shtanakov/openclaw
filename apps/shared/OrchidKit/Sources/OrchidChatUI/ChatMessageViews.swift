...
struct ChatMessageBubble: View {
    let message: OrchidChatMessage
    let style: OrchidChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? OrchidChatTheme.userText : OrchidChatTheme.assistantText
    ...
    private var inlineAttachments: [OrchidChatMessageContent] {
    ...
    private var toolCalls: [OrchidChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [OrchidChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? OrchidChatTheme.userBubble
        }
        if self.style == .onboarding {
            return OrchidChatTheme.onboardingAssistantBubble
        }
        return OrchidChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return OrchidChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: OrchidChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? OrchidChatTheme.userText : OrchidChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: OrchidChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OrchidChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(OrchidChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [OrchidChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: OrchidChatTheme.assistantText)
            }
        }
    }
}
