...
@MainActor
struct OrchidChatComposer: View {
    @Bindable var viewModel: OrchidChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(OrchidChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(OrchidChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \OrchidPendingAttachment.id)
                { (att: OrchidPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            OrchidPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OrchidChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(OrchidChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(OrchidChatTheme.subtleCard)
        .clipShape(Capsule())
    }
