...
private let chatUILogger = Logger(subsystem: "ai.orchid", category: "OrchidChatUI")

@MainActor
@Observable
public final class OrchidChatViewModel {
    public private(set) var messages: [OrchidChatMessage] = []
    ...
    public var attachments: [OrchidPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OrchidChatPendingToolCall] = []
    public private(set) var sessions: [OrchidChatSessionEntry] = []
    private let transport: any OrchidChatTransport
    ...
    private var pendingToolCallsById: [String: OrchidChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any OrchidChatTransport) {
    ...
    public var sessionChoices: [OrchidChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [OrchidChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OrchidPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OrchidChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OrchidChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: OrchidChatMessage) -> OrchidChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> OrchidChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return OrchidChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: OrchidChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [OrchidChatMessage],
        incoming: [OrchidChatMessage]) -> [OrchidChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [OrchidChatMessage]) -> [OrchidChatMessage] {
        var result: [OrchidChatMessage] = []
    ...
    private static func dedupeKey(for message: OrchidChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OrchidChatMessageContent] = [
            OrchidChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> OrchidChatAttachmentPayload in
            OrchidChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OrchidChatSessionEntry {
        OrchidChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: OrchidChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OrchidChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OrchidAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OrchidChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OrchidPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
