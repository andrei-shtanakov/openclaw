...
final class MotionService: MotionServicing {
    func activities(params: OrchidMotionActivityParams) async throws -> OrchidMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OrchidMotionActivityEntry], Error>) in
        ...
        return OrchidMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: OrchidPedometerParams) async throws -> OrchidPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<OrchidPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = OrchidPedometerPayload(
