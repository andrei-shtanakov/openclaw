...
final class ContactsService: ContactsServicing {
    ...
    func search(params: OrchidContactsSearchParams) async throws -> OrchidContactsSearchPayload {
        ...
        return OrchidContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: OrchidContactsAddParams) async throws -> OrchidContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return OrchidContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return OrchidContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> OrchidContactPayload {
        OrchidContactPayload(
