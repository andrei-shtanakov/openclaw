...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> OrchidNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> OrchidNetworkStatusPayload {
        let status: OrchidNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> OrchidNetworkStatusPayload {
        OrchidNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
