...
struct IOSGatewayChatTransport: OrchidChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.orchid", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> OrchidChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OrchidChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OrchidChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OrchidChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OrchidChatAttachmentPayload]) async throws -> OrchidChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OrchidChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(OrchidChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(OrchidGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OrchidGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OrchidChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OrchidAgentEventPayload.self)
                        {
