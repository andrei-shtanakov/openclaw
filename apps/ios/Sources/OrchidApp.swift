...
@MainActor
final class OrchidAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.orchid.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.orchid.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.orchid.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "orchid.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "orchid.watch.promptId"
    static let sessionKeyKey = "orchid.watch.sessionKey"
    static let actionPrimaryIDKey = "orchid.watch.action.primary.id"
    static let actionPrimaryLabelKey = "orchid.watch.action.primary.label"
    static let actionSecondaryIDKey = "orchid.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "orchid.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "orchid.watch.action.primary"
    static let actionSecondaryIdentifier = "orchid.watch.action.secondary"
    static let categoryPrefix = "orchid.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: OrchidWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> OrchidWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return OrchidWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Orchid" : title
        content.body = body
    ...
    private static func categoryActions(
        primaryAction: OrchidWatchAction,
        secondaryAction: OrchidWatchAction?) -> [UNNotificationAction]
    {
...
@main
struct OrchidApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(OrchidAppDelegate.self) private var appDelegate
...
extension OrchidApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Orchid: installing uncaught exception handler")
