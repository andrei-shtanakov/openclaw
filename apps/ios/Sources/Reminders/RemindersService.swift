...
final class RemindersService: RemindersServicing {
    func list(params: OrchidRemindersListParams) async throws -> OrchidRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OrchidReminderPayload], Error>) in
        ...
        return OrchidRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OrchidRemindersAddParams) async throws -> OrchidRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OrchidReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OrchidRemindersAddPayload(reminder: payload)
    }
