...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.orchid.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Orchid", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Orchid gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Orchid.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "OrchidShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Orchid is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Orchid Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("orchid-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
