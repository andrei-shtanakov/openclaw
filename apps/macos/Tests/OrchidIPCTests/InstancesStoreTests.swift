...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: OrchidProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OrchidProtocol.AnyCodable] = [
            "presence": .init([OrchidProtocol.AnyCodable(entry)]),
        ]
        let payload = OrchidProtocol.AnyCodable(payloadMap)
