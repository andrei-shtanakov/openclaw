...
struct OrchidMacCLI {
    static func main() async {
        ...
        default:
            fputs("orchid-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      orchid-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      orchid-mac discover [--timeout <ms>] [--json] [--include-local]
      orchid-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      orchid-mac connect
      orchid-mac connect --url ws://127.0.0.1:18789 --json
      orchid-mac discover --timeout 3000 --json
      orchid-mac wizard --mode local
    """)
}
