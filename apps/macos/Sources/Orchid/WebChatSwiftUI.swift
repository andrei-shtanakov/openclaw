...
private let webChatSwiftLogger = Logger(subsystem: "ai.orchid", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: OrchidChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OrchidChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(OrchidChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OrchidChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OrchidGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OrchidChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OrchidChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OrchidChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: OrchidChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OrchidChatView>) -> NSViewController
    {
