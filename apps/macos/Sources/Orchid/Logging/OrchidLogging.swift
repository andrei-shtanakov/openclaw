...
enum OrchidLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = OrchidOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OrchidFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OrchidLogging.bootstrapIfNeeded()
        let label = OrchidLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OrchidOSLogHandler: LogHandler {
...
struct OrchidFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OrchidLogging.parseLabel(self.label)
