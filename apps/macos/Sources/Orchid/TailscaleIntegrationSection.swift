...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.orchid/orchid.json. Prefer ORCHID_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.orchid/orchid.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.orchid/orchid.json. Restart the gateway to apply."
        }
