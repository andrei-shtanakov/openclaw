...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.orchid.mac`) even if the shipped
    ...
    {
        if environment["ORCHID_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "orchid.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "orchid.nixMode") { return true }
