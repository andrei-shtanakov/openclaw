...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.orchid", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["ORCHID_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = OrchidConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.orchidExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "orchid CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["ORCHID_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "orchid@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "orchid@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "orchid@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "orchid@\(target)"])
            }

        statusHandler("Installing orchid@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed orchid@\(target)")
        } else {
