...
enum DebugActions {
    private static let verboseDefaultsKey = "orchid.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "orchid.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OrchidPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Orchid", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OrchidPaths.configURL
        guard
