...
enum CommandResolver {
    private static let projectRootDefaultsKey = "orchid.gatewayProjectRootPath"
    private static let helperName = "orchid"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let orchidEntry = root.appendingPathComponent("orchid.mjs").path
        if FileManager().isReadableFile(atPath: orchidEntry) { return orchidEntry }
        let binEntry = root.appendingPathComponent("bin/orchid.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/orchid")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let orchidPaths = self.orchidManagedPaths(home: home)
        if !orchidPaths.isEmpty {
            extras.insert(contentsOf: orchidPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + orchidPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func orchidManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".orchid"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func orchidExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOrchidExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("orchid.mjs").path,
            root.appendingPathComponent("bin/orchid.js").path,
        ]
    ...
    static func hasAnyOrchidInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.orchidExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let orchidPath = self.projectOrchidExecutable(projectRoot: root) {
                return [orchidPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "orchid", subcommand] + extraArgs
            }
            if let orchidPath = self.orchidExecutable(searchPaths: searchPaths) {
                return [orchidPath, subcommand] + extraArgs
            }

            let missingEntry = """
            orchid entrypoint missing (looked for dist/index.js or orchid.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real orchid CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/orchid"
        ...
        \(cliSection)
        \(projectSection)
        if command -v orchid >/dev/null 2>&1; then
          CLI="$(command -v orchid)"
          orchid \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/orchid.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/orchid.mjs"
            node "$PRJ/orchid.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/orchid.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/orchid.js"
            node "$PRJ/bin/orchid.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent orchid"
          pnpm --silent orchid \(quotedArgs);
        else
          echo "orchid CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OrchidConfigFile.loadDict()
