...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Orchid token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: OrchidOAuthStore.AnthropicOAuthStatus = OrchidOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.orchid", category: "anthropic-oauth")
...
enum OrchidOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let orchidOAuthDirEnv = "ORCHID_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Orchid OAuth token file not found"
            case .unreadableFile: "Orchid OAuth token file not readable"
            case .invalidJSON: "Orchid OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Orchid OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Orchid OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.orchidOAuthDirEnv]?
