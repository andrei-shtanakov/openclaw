...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ORCHID_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/orchid")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("orchid-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("orchid-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/orchid/ (rolling or stdout), or nil if none exist.
