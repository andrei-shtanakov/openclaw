...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OrchidProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.orchid", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OrchidKit.AnyCodable]()) {
                $0[$1.key] = OrchidKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OrchidProtocol.AnyCodable?) -> [String: OrchidProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OrchidProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OrchidKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OrchidProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("orchid.control.heartbeat")
    static let controlAgentEvent = Notification.Name("orchid.control.agent")
}
