...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.orchid", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: OrchidPeekabooBridgeServices?
    private static var orchidSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Orchid", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = OrchidPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.orchidSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.orchidSocketPath, privacy: .public)")
    }
...
@MainActor
private final class OrchidPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.orchid.peekaboo")
