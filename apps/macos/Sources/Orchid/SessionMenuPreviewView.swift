...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.orchid", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OrchidSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OrchidSessionPreviewEntry,
    ...
    private static func cache(payload: OrchidSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OrchidChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OrchidKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OrchidKit.AnyCodable]) -> [OrchidChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OrchidChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OrchidChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OrchidChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OrchidChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OrchidChatMessage) -> String? {
