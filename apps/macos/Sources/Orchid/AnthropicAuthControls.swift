...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: OrchidOAuthStore.AnthropicOAuthStatus = OrchidOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: OrchidOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = OrchidOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = OrchidOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try OrchidOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Orchid can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: OrchidOAuthStore.AnthropicOAuthStatus,
