...
INSTALL_URL="${ORCHID_INSTALL_URL:-https://orchid.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ORCHID_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ORCHID_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="orchid"
PACKAGE_NAME="${ORCHID_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ORCHID_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ORCHID_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ORCHID_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
