...
IMAGE_NAME="orchid-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="orchid-net-e2e-$$"
GW_NAME="orchid-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "ORCHID_GATEWAY_TOKEN=$TOKEN" \
	  -e "ORCHID_SKIP_CHANNELS=1" \
	  -e "ORCHID_SKIP_GMAIL_WATCHER=1" \
	  -e "ORCHID_SKIP_CRON=1" \
	  -e "ORCHID_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
