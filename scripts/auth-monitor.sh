...
#
# Suggested cron: */30 * * * * /home/admin/orchid/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Orchid notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., orchid-alerts)
...
STATE_FILE="$HOME/.orchid/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Orchid if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use orchid
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Orchid to $NOTIFY_PHONE..."
            orchid send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Orchid is down. Run: ssh l36 '~/orchid/scripts/mobile-reauth.sh'" "urgent"
    exit 1
