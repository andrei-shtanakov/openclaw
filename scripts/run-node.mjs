...
const shouldBuild = (deps) => {
  if (deps.env.ORCHID_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.ORCHID_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[orchid] ${message}\n`);
};

const runOrchid = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["orchid.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
