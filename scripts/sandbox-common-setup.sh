...
BASE_IMAGE="${BASE_IMAGE:-orchid-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-orchid-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=orchid.sandbox=1)
NOTE
