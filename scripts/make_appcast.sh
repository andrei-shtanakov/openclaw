...
ZIP=${1:?"Usage: $0 Orchid-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/orchid/orchid/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Orchid-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/orchid/orchid/releases/download/v${VERSION}/"}
