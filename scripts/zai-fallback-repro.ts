...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "orchid.json");
  ...
  const sessionId =
    process.env.ORCHID_ZAI_FALLBACK_SESSION_ID ??
    process.env.ORCHID_ZAI_FALLBACK_SESSION_ID ??
    randomUUID();
  ...
  const run1 = await runCommand(
    "run1",
    ["orchid", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["orchid", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
