...
# Check auth status
termux-toast "Checking Orchid auth..."

STATUS=$(ssh "$SERVER" '$HOME/orchid/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/orchid/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|ORCHID_EXPIRING|ORCHID_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/orchid/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|ORCHID_EXPIRED|ORCHID_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Orchid won't work until re-auth"
                ;;
        esac
        ;;
