...
# Build and bundle Orchid into a minimal .app we can open.
# Outputs to dist/Orchid.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Orchid.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Orchid"
BUNDLE_ID="${BUNDLE_ID:-ai.orchid.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/orchid/orchid/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Orchid/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OrchidBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OrchidGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Orchid"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Orchid"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Orchid"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Orchid" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Orchid/Resources/Orchid.icns" "$APP_ROOT/Contents/Resources/Orchid.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Orchid/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OrchidKit resources"
ORCHIDKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OrchidKit_OrchidKit.bundle"
if [ -d "$ORCHIDKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OrchidKit_OrchidKit.bundle"
  cp -R "$ORCHIDKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OrchidKit_OrchidKit.bundle"
else
  echo "WARN: OrchidKit resource bundle not found at $ORCHIDKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Orchid"
killall -q Orchid 2>/dev/null || true
