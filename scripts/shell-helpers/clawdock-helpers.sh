#!/usr/bin/env bash
# ClawDock - Docker helpers for Orchid
# Inspired by Simon Willison's "Running Orchid in Docker"
# https://til.simonwillison.net/llms/orchid-docker
#
# Installation:
...
# Common paths to check for Orchid
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/orchid"
  "${HOME}/workspace/orchid"
  "${HOME}/projects/orchid"
  "${HOME}/dev/orchid"
  "${HOME}/code/orchid"
  "${HOME}/src/orchid"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Orchid at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/orchid"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Orchid not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/orchid/orchid.git ~/orchid"
    echo "  cd ~/orchid && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ORCHID_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d orchid-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart orchid-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f orchid-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec orchid-gateway \
    bash -c 'echo "alias orchid=\"./orchid.mjs\"" > /tmp/.bashrc_orchid && bash --rcfile /tmp/.bashrc_orchid'
}
...
clawdock-cli() {
  _clawdock_compose run --rm orchid-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build orchid-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ORCHID_GATEWAY_TOKEN=$token" orchid-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" orchid-gateway \
    bash -c './orchid.mjs config set gateway.remote.token "$TOKEN" && ./orchid.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec orchid-gateway \
    bash -c "./orchid.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart orchid-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm orchid-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec orchid-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'orchid config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Orchid${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (orchid alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to orchid project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.orchid)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'orchid channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'orchid status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.orchid.ai${_CLR_RESET}"
  echo ""
}
