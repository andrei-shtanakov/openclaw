...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/orchid/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Orchid Re-Auth" -c "After getting key, run: ssh $SERVER '~/orchid/scripts/mobile-reauth.sh'" --id orchid-auth
        ;;
    *)
