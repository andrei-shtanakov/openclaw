#!/bin/bash
# Setup Orchid Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'orchid-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Orchid message: Send warning via Orchid itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/orchid-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/orchid-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/orchid-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now orchid-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status orchid-auth-monitor.timer"
echo ""
echo "Quick commands:"
