 ...
 * `orchid update`. Use it to verify the completion installation flow
...
\`orchid update\`, \`orchid doctor\`, and \`orchid onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Orchid Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`orchid completion --install\` later to enable.`));
    return;
  }
