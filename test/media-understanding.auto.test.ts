...
import type { OrchidConfig } from "../src/config/config.js";
import { resolvePreferredOrchidTmpDir } from "../src/infra/tmp-orchid-dir.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("orchid-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it.skipIf(process.platform === "win32")("uses sherpa-onnx-offline when available", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "orchid-bin-sherpa-");
      const modelDir = await createTrackedTempDir(tempPaths, "orchid-sherpa-model-");
      ...
      const cfg: OrchidConfig = { tools: { media: { audio: {} } } };
  ...
  it.skipIf(process.platform === "win32")("uses whisper-cli when sherpa is missing", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "orchid-bin-whispercpp-");
      const modelDir = await createTrackedTempDir(tempPaths, "orchid-whispercpp-model-");
  ...
  it.skipIf(process.platform === "win32")("uses gemini CLI for images when available", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "orchid-bin-gemini-");
      ...
      const cfg: OrchidConfig = { tools: { media: { image: {} } } };
  ...
  it("skips auto-detect when no supported binaries are available", async () => {
    await withEnvSnapshot(async () => {
      const emptyBinDir = await createTrackedTempDir(tempPaths, "orchid-bin-empty-");
