...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    orchidHome: process.env.ORCHID_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ORCHID_HOME", snapshot.orchidHome);
  restoreKey("ORCHID_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ORCHID_HOME.
  delete process.env.ORCHID_HOME;
  process.env.ORCHID_STATE_DIR = path.join(base, ".orchid");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "orchid-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".orchid", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
