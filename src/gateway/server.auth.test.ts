...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.ORCHID_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.ORCHID_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.ORCHID_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.ORCHID_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
    ...
    test("connect (req) handshake prefers ORCHID_VERSION over service version", async () => {
      await withRuntimeVersionEnv(
        {
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `orchid-test-device-${randomUUID()}.json`),
        nonce,
      });
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.ORCHID_GATEWAY_TOKEN;
      process.env.ORCHID_GATEWAY_TOKEN = "secret";
      port = await getFreePort();
      server = await startGatewayServer(port);
    });
    ...
    try {
      await withGatewayServer(async ({ port }) => {
        ...
        const { device } = await createSignedDevice({
          ...
          identityPath: path.join(os.tmpdir(), `orchid-controlui-device-${randomUUID()}.json`),
          nonce: String(nonce),
        });
        const res = await connectReq(ws, {
          token: "secret",
  ...
  test("requires pairing for scope upgrades", async () => {
    ...
    const { getPairedDevice } = await import("../infra/device-pairing.js");
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "orchid-device-scope-"));
  ...
  test("allows legacy paired devices missing role/scope metadata", async () => {
    ...
    const { getPairedDevice } = await import("../infra/device-pairing.js");
    const identityDir = await mkdtemp(join(tmpdir(), "orchid-device-legacy-meta-"));
    const identity = loadOrCreateDeviceIdentity(join(identityDir, "device.json"));
  ...
  test("rejects scope escalation from legacy paired metadata", async () => {
    ...
    let ws2: WebSocket | undefined;
    try {
      const identityDir = await mkdtemp(join(tmpdir(), "orchid-device-legacy-"));
      const identity = loadOrCreateDeviceIdentity(join(identityDir, "device.json"));
      const client = {
        id: GATEWAY_CLIENT_NAMES.TEST,
        version: "1.0.0",
