...
import { resolveOrchidAgentDir } from "../agents/agent-paths.js";
...
import type { ModelsConfig, OrchidConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ORCHID_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ORCHID_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ORCHID_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<OrchidConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.ORCHID_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.ORCHID_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveOrchidAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-live-state-"));
  process.env.ORCHID_STATE_DIR = tempStateDir;
  ...
  process.env.ORCHID_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.orchid-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveOrchidAgentDir();
  const sanitizedCfg: OrchidConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Orchid live tool probe (local, safe): " +
    ...
    process.env.ORCHID_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ORCHID_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureOrchidModelsJson(cfg);
      ...
      const rawModels = process.env.ORCHID_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.orchid-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
