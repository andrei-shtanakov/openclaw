...
import type { OrchidConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
const BASE_RELOAD_RULES_TAIL: ReloadRule[] = [
  ...
  { prefix: "models", kind: "hot", actions: ["reload-models"] },
...
export function resolveGatewayReloadSettings(cfg: OrchidConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: OrchidConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: OrchidConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: OrchidConfig) => void;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: OrchidConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: OrchidConfig) => {
