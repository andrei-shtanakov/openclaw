...
const { createOrchidTools } = await import("../agents/orchid-tools.js");
...
type SessionSendTool = ReturnType<typeof createOrchidTools>[number];

function getSessionsSendTool(): SessionSendTool {
  const tool = createOrchidTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ORCHID_GATEWAY_PORT", "ORCHID_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.ORCHID_GATEWAY_PORT = String(gatewayPort);
  process.env.ORCHID_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    const configPath = process.env.ORCHID_CONFIG_PATH;
    if (!configPath) {
      throw new Error("ORCHID_CONFIG_PATH missing in gateway test environment");
    }
