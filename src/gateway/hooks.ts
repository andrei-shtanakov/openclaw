...
export function resolveHooksConfig(cfg: OrchidConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OrchidConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-orchid-token"] === "string" ? req.headers["x-orchid-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
