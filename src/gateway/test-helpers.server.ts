...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "ORCHID_STATE_DIR",
  "ORCHID_CONFIG_PATH",
  "ORCHID_SKIP_BROWSER_CONTROL_SERVER",
  "ORCHID_SKIP_GMAIL_WATCHER",
  "ORCHID_SKIP_CANVAS_HOST",
  "ORCHID_BUNDLED_PLUGINS_DIR",
  "ORCHID_SKIP_CHANNELS",
  "ORCHID_SKIP_PROVIDERS",
  "ORCHID_SKIP_CRON",
  "ORCHID_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ORCHID_STATE_DIR = path.join(tempHome, ".orchid");
  delete process.env.ORCHID_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.ORCHID_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.ORCHID_SKIP_GMAIL_WATCHER = "1";
  process.env.ORCHID_SKIP_CANVAS_HOST = "1";
  process.env.ORCHID_SKIP_CHANNELS = "1";
  process.env.ORCHID_SKIP_PROVIDERS = "1";
  process.env.ORCHID_SKIP_CRON = "1";
  process.env.ORCHID_TEST_MINIMAL_GATEWAY = "1";
  process.env.ORCHID_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "orchid-test-no-bundled-extensions")
    : "orchid-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "orchid-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".orchid-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__orchidTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__orchidTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["ORCHID_GATEWAY_TOKEN"]);
  const prev = process.env.ORCHID_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ORCHID_GATEWAY_TOKEN = fallbackToken;
  }
