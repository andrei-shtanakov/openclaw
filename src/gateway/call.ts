...
export function buildGatewayConnectionDetails(
  options: { config?: OrchidConfig; url?: string; configPath?: string } = {},
...
function resolveGatewayCredentials(context: ResolvedGatewayCallContext): {
  ...
  const token =
    ...
    (!context.urlOverride
      ? context.isRemoteMode
        ...
        : trimToUndefined(process.env.ORCHID_GATEWAY_TOKEN) ||
          trimToUndefined(process.env.ORCHID_GATEWAY_TOKEN) ||
  ...
  const password =
    ...
    (!context.urlOverride
      ? trimToUndefined(process.env.ORCHID_GATEWAY_PASSWORD) ||
        trimToUndefined(process.env.ORCHID_GATEWAY_PASSWORD) ||
        (context.isRemoteMode
