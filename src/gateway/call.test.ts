...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("orchid doctor --fix");
  });
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      label: "password",
      authKey: "password",
      envKey: "ORCHID_GATEWAY_PASSWORD",
      envValue: "from-env",
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ORCHID_GATEWAY_PASSWORD", "ORCHID_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.ORCHID_GATEWAY_PASSWORD = envPassword;
    }
