...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".orchid");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ORCHID_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ORCHID_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ORCHID_STATE_DIR.
