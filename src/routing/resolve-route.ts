...
function listAgents(cfg: OrchidConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: OrchidConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OrchidConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<OrchidConfig, EvaluatedBindingsCache>();
