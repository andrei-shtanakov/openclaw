...
homepage: https://docs.orchid.ai/automation/hooks#command-logger
metadata:
  {
    "orchid":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.orchid/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.orchid/logs/commands.log ~/.orchid/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/orchid`:
   ```
   /home/username/.orchid/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
