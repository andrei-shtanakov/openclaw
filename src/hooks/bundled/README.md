...
This directory contains hooks that ship with Orchid. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.orchid/orchid.json`:
...
2. Restart gateway: `pkill -9 -f 'orchid.*gateway' && pnpm orchid gateway`
3. Enable the hook: `orchid hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.orchid.ai/automation/hooks
