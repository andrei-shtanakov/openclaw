...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => OrchidConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("orchid-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): OrchidConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
