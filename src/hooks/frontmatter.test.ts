...
`;
    ...
    expect(parsed.orchid.emoji).toBe("💾");
    expect(parsed.orchid.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.orchid.emoji).toBe("📝");
    expect(parsed.orchid.events).toEqual(["command"]);
    expect(parsed.orchid.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.orchid.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"orchid": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"orchid": {"events": ["test"]}}');
  });
...
describe("resolveOrchidMetadata", () => {
  it("extracts orchid metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOrchidMetadata(frontmatter);
  ...
  it("returns undefined when orchid key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        orchid: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Orchid" },
            { id: "npm", kind: "npm", package: "@orchid/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@orchid/hook");
  });
    ...
    "orchid":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Orchid" }],
      },
  }
---
...
`;
    ...
    const orchid = resolveOrchidMetadata(frontmatter);
    expect(orchid).toBeDefined();
    expect(orchid?.emoji).toBe("💾");
    expect(orchid?.events).toEqual(["command:new", "command:reset"]);
    expect(orchid?.requires?.config).toEqual(["workspace.dir"]);
    expect(orchid?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const orchid = resolveOrchidMetadata(frontmatter);
    expect(orchid?.emoji).toBe("disk");
    expect(orchid?.events).toEqual(["command:new"]);
  });
});
