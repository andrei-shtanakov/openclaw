...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ORCHID_BUNDLED_HOOKS_DIR"]);
    process.env.ORCHID_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
