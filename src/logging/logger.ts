...
export const DEFAULT_LOG_DIR = resolvePreferredOrchidTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "orchid.log"); // legacy single-file path

const LOG_PREFIX = "orchid";
...
function resolveSettings(): ResolvedSettings {
  let cfg: OrchidConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => OrchidConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.ORCHID_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[orchid] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
