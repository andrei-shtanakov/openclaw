...
const testLogPath = path.join(os.tmpdir(), "orchid-test-env-log-level.log");
const defaultMaxFileBytes = 500 * 1024 * 1024;

describe("ORCHID_LOG_LEVEL", () => {
  let originalEnv: string | undefined;

  beforeEach(() => {
    originalEnv = process.env.ORCHID_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.ORCHID_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.ORCHID_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("ORCHID_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid ORCHID_LOG_LEVEL="nope"');
  });
});
