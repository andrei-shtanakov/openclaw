...
function createMemorySearchCfg(options: { workspaceDir: string; indexPath: string }): OrchidConfig {
  return {
    agents: {
      defaults: {
...
describe("MemoryIndexManager.readFile", () => {
  ...
  beforeEach(async () => {
    resetEmbeddingMocks();
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-mem-read-"));
