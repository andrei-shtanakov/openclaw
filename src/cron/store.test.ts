...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ORCHID_HOME for tilde expansion", () => {
    vi.stubEnv("ORCHID_HOME", "/srv/orchid-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/orchid-home", "cron", "jobs.json"));
  });
});
