...
import type { OrchidConfig, ConfigFileSnapshot } from "../config/types.orchid.js";
...
vi.mock("../infra/orchid-root.js", () => ({
  resolveOrchidPackageRoot: vi.fn(),
}));
...
const { resolveOrchidPackageRoot } = await import("../infra/orchid-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/orchid-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as OrchidConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/orchid-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveOrchidPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("orchid-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveOrchidPackageRoot).mockClear();
    ...
    vi.mocked(resolveOrchidPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("orchid");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/orchid-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "orchid-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Orchid update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as OrchidConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveOrchidPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("orchid-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ ORCHID_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.ORCHID_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("orchid-update-wizard");
    await withEnvAsync({ ORCHID_GIT_DIR: tempDir }, async () => {
      setTty(true);
