    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["orchid nodes status", "List known nodes with live status."],
          ["orchid nodes pairing pending", "Show pending node pairing requests."],
          ['orchid nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["orchid nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.orchid.ai/cli/nodes")}\n`,
    );
