    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["orchid memory status", "Show index and provider status."],
          ["orchid memory index --force", "Force a full reindex."],
          ['orchid memory search --query "deployment notes"', "Search indexed memory entries."],
          ["orchid memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.orchid.ai/cli/memory")}\n`,
    );
