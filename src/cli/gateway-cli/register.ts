      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["orchid gateway run", "Run the gateway in the foreground."],
            ["orchid gateway status", "Show service status and probe reachability."],
            ["orchid gateway discover", "Find local and wide-area gateway beacons."],
            ["orchid gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.orchid.ai/cli/gateway")}\n`,
      ),
  );
