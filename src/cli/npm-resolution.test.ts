...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@orchid/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@orchid/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@orchid/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@orchid/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @orchid/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@orchid/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@orchid/plugin-alpha@1.2.3",
        installPath: "/tmp/orchid/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@orchid/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @orchid/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@orchid/plugin-alpha@latest",
      true,
      "/tmp/orchid/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@orchid/plugin-alpha@latest",
      installPath: "/tmp/orchid/extensions/alpha",
      version: "1.2.3",
