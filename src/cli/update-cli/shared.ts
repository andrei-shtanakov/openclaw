...
import { resolveOrchidPackageRoot } from "../../infra/orchid-root.js";
...
const ORCHID_REPO_URL = "https://github.com/orchid/orchid.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "orchid";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("orchid@")) {
    return trimmed.slice("orchid@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.ORCHID_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveOrchidPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", ORCHID_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `ORCHID_GIT_DIR points at a non-git directory: ${params.dir}. Set ORCHID_GIT_DIR to an empty folder or an orchid checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`ORCHID_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "orchid.mjs");
