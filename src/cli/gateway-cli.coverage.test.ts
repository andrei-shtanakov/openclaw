...
describe("gateway-cli coverage", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedOutput: [
        "Gateway Discovery",
        "Found 1 gateway(s)",
        "- Studio orchid.internal.",
        "  tailnet: studio.tailnet.ts.net",
        "  host: studio.orchid.internal",
        "  ws: ws://studio.orchid.internal:18789",
      ],
    },
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ ORCHID_GATEWAY_PORT: "19001" }, async () => {
      resetRuntimeCapture();
