...
${formatHelpExamples([
  ['orchid agent --to +15555550123 --message "status update"', "Start a new session."],
  ['orchid agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'orchid agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'orchid agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['orchid agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'orchid agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.orchid.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.orchid.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['orchid agents set-identity --agent main --name "Orchid" --emoji "🦞"', "Set name + emoji."],
  ["orchid agents set-identity --agent main --avatar avatars/orchid.png", "Set avatar path."],
  [
    "orchid agents set-identity --workspace ~/.orchid/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "orchid agents set-identity --identity-file ~/.orchid/workspace/IDENTITY.md --agent main",
