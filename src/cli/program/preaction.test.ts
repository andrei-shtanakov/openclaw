...
beforeEach(() => {
  ...
  originalHideBanner = process.env.ORCHID_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.ORCHID_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.ORCHID_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("orchid");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "orchid", "status", "--debug"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "orchid", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "orchid", "completion"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "orchid", "--version"],
    });
  ...
  it("hides banner when ORCHID_HIDE_BANNER is truthy", async () => {
    process.env.ORCHID_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "orchid", "status"],
    });
