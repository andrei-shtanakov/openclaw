...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "orchid-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "orchid-bundled-skills-test-"));
    process.env.ORCHID_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
