...
import type { ConfigFileSnapshot, OrchidConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `orchid config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: OrchidConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: OrchidConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: OrchidConfig, config: OrchidConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: OrchidConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: OrchidConfig = { gateway: { port: 18789 } };
