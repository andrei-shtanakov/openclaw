...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Orchid and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["orchid update", "Update a source checkout (git)"],
        ["orchid update --channel beta", "Switch to beta channel (git + npm)"],
        ["orchid update --channel dev", "Switch to dev channel (git + npm)"],
        ["orchid update --tag beta", "One-off update to a dist-tag or version"],
        ["orchid update --dry-run", "Preview actions without changing anything"],
        ["orchid update --no-restart", "Update without restarting the service"],
        ["orchid update --json", "Output result as JSON"],
        ["orchid update --yes", "Non-interactive (accept downgrade prompts)"],
        ["orchid update wizard", "Interactive update wizard"],
        ["orchid --update", "Shorthand for orchid update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run orchid update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.orchid.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["orchid update status", "Show channel + version status."],
          ["orchid update status --json", "JSON output."],
          ["orchid update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.orchid.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
