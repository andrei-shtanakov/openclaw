...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("orchid node install"), formatCliCommand("orchid node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("orchid node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ORCHID_LOG_PREFIX: baseEnv.ORCHID_LOG_PREFIX ?? "node",
