...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("orchid-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("orchid-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("orchid-lid-a-", (first) => {
      withTempDirSync("orchid-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.orchid when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "orchid-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers ORCHID_HOME over HOME", () => {
    vi.stubEnv("ORCHID_HOME", "/srv/orchid-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/orchid-home"));
...
describe("shortenHomePath", () => {
  it("uses $ORCHID_HOME prefix when ORCHID_HOME is set", () => {
    vi.stubEnv("ORCHID_HOME", "/srv/orchid-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/orchid-home")}/.orchid/orchid.json`)).toBe(
      "$ORCHID_HOME/.orchid/orchid.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $ORCHID_HOME replacement when ORCHID_HOME is set", () => {
    vi.stubEnv("ORCHID_HOME", "/srv/orchid-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/orchid-home")}/.orchid/orchid.json`),
    ).toBe("config: $ORCHID_HOME/.orchid/orchid.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/orchid")).toBe(path.resolve(os.homedir(), "orchid"));
  });
  ...
  it("prefers ORCHID_HOME for tilde expansion", () => {
    vi.stubEnv("ORCHID_HOME", "/srv/orchid-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/orchid")).toBe(path.resolve("/srv/orchid-home", "orchid"));
