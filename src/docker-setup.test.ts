...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "orchid-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ORCHID_GATEWAY_TOKEN: "test-token",
    ORCHID_CONFIG_DIR: join(sandbox.rootDir, "config"),
    ORCHID_WORKSPACE_DIR: join(sandbox.rootDir, "orchid"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ORCHID_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      ORCHID_EXTRA_MOUNTS: undefined,
      ORCHID_HOME_VOLUME: "orchid-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ORCHID_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("ORCHID_EXTRA_MOUNTS=");
    expect(envFile).toContain("ORCHID_HOME_VOLUME=orchid-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("orchid-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("orchid-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg ORCHID_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });

  it("rejects injected multiline ORCHID_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ORCHID_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ORCHID_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid ORCHID_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ORCHID_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid ORCHID_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ORCHID_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ORCHID_HOME_VOLUME must match");
  });
