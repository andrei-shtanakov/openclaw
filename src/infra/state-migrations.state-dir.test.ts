...
async function makeTempRoot() {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "orchid-state-dir-"));
  tempRoot = root;
  return root;
}
...
describe("legacy state dir auto-migration", () => {
  it("follows legacy symlink when it points at another legacy dir (openclaw -> moltbot)", async () => {
    ...
    const targetMarker = path.join(root, ".orchid", "marker.txt");
