...
function candidateBinDirs(opts: EnsureOrchidPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `orchid` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "orchid");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ORCHID_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "orchid"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `orchid` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOrchidCliOnPath(opts: EnsureOrchidPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ORCHID_PATH_BOOTSTRAPPED)) {
    return;
  }
