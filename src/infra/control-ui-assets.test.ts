...
vi.mock("./orchid-root.js", () => ({
  resolveOrchidPackageRoot: vi.fn(async () => null),
  resolveOrchidPackageRootSync: vi.fn(() => null),
}));
...
let orchidRoot: typeof import("./orchid-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    orchidRoot = await import("./orchid-root.js");
  });
  ...
  it("uses resolveOrchidPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/orchid");
    (
      orchidRoot.resolveOrchidPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/orchid"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "orchid" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "orchid.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-orchid");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/orchid-bundle");
    (
      orchidRoot.resolveOrchidPackageRootSync as unknown as ReturnType<typeof vi.fn>
