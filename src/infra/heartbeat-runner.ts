...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: OrchidConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: OrchidConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: OrchidConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: OrchidConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: OrchidConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: OrchidConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: OrchidConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: OrchidConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
