...
const DEFAULT_PACKAGE_NAME = "orchid";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "orchid.mjs");
    ...
    const doctorStep = await runStep(
      step("orchid doctor", doctorArgv, gitRoot, { ORCHID_UPDATE_IN_PROGRESS: "1" }),
    );
