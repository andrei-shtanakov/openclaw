...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__orchid_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "orchid-root");
...
describe("resolveOrchidPackageRoot", () => {
  let resolveOrchidPackageRoot: typeof import("./orchid-root.js").resolveOrchidPackageRoot;
  let resolveOrchidPackageRootSync: typeof import("./orchid-root.js").resolveOrchidPackageRootSync;

  beforeAll(async () => {
    ({ resolveOrchidPackageRoot, resolveOrchidPackageRootSync } = await import("./orchid-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "orchid");
    const pkgRoot = path.join(project, "node_modules", "orchid");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "orchid" }));

    expect(resolveOrchidPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "orchid");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "orchid.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "orchid" }));

    expect(resolveOrchidPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveOrchidPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-orchid package roots", async () => {
    const pkgRoot = fx("not-orchid");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-orchid" }));

    expect(resolveOrchidPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveOrchidPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveOrchidPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
