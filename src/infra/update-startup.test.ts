...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ORCHID_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.ORCHID_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveOrchidPackageRoot } = await import("./orchid-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveOrchidPackageRoot).mockResolvedValue("/opt/orchid");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/orchid/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
