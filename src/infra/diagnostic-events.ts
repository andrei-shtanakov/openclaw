...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __orchidDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__orchidDiagnosticEventsState) {
    globalStore.__orchidDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__orchidDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: OrchidConfig): boolean {
