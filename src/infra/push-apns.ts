...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ORCHID_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ORCHID_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ORCHID_APNS_TEAM_ID and ORCHID_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ORCHID_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ORCHID_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ORCHID_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ORCHID_APNS_PRIVATE_KEY_P8 or ORCHID_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ORCHID_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createOrchidPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    orchid: createOrchidPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
