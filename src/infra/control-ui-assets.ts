...
import { resolveOrchidPackageRoot, resolveOrchidPackageRootSync } from "./orchid-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveOrchidPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "orchid" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveOrchidPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
