...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ORCHID_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*orchid-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "orchid";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "orchid");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "orchid-gw");
  ...
  records.push(`_orchid-gw._tcp IN PTR ${instanceLabel}._orchid-gw._tcp`);
  records.push(`${instanceLabel}._orchid-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._orchid-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; orchid-content-hash: ${contentHash}\n${contentBody}`;
}
