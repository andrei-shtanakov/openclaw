...
export function triggerOrchidRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.ORCHID_SYSTEMD_UNIT,
        process.env.ORCHID_PROFILE,
      );
  ...
  const label =
    process.env.ORCHID_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.ORCHID_PROFILE);
