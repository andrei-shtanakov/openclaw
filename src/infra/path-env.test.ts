...
let ensureOrchidCliOnPath: typeof import("./path-env.js").ensureOrchidCliOnPath;

describe("ensureOrchidCliOnPath", () => {
  const envKeys = [
    "PATH",
    "ORCHID_PATH_BOOTSTRAPPED",
    "ORCHID_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOrchidCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling orchid exists", () => {
    const tmp = abs("/tmp/orchid-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "orchid");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.ORCHID_PATH_BOOTSTRAPPED;

    ensureOrchidCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.ORCHID_PATH_BOOTSTRAPPED = "1";
    ensureOrchidCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/orchid-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "orchid");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/orchid-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "orchid");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "orchid");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/orchid-path/case-linuxbrew");
