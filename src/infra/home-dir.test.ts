...
describe("resolveEffectiveHomeDir", () => {
  it("prefers ORCHID_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/orchid-home"));
  });
  ...
  it("expands ORCHID_HOME when set to ~", () => {
    const env = {
      ORCHID_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for ORCHID_HOME", () => {
    const result = resolveRequiredHomeDir(
      { ORCHID_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when ORCHID_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ ORCHID_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { ORCHID_HOME: "/srv/orchid-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/orchid-home")}/x`);
  });
