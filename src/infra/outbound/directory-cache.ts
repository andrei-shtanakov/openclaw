...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OrchidConfig | null = null;
  ...
  get(key: string, cfg: OrchidConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OrchidConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: OrchidConfig): void {
