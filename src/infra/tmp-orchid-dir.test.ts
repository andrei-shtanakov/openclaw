...
import { POSIX_ORCHID_TMP_DIR, resolvePreferredOrchidTmpDir } from "./tmp-orchid-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOrchidTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `orchid-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredOrchidTmpDir({
    accessSync,
...
describe("resolvePreferredOrchidTmpDir", () => {
  it("prefers /tmp/orchid when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_ORCHID_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/orchid when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ORCHID_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/orchid when /tmp/orchid is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/orchid when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/orchid is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/orchid is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/orchid is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
