...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ORCHID_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Orchid";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Orchid\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.ORCHID_MDNS_HOSTNAME?.trim() ||
    process.env.ORCHID_MDNS_HOSTNAME?.trim() ||
    "orchid";
  const hostname =
    hostnameRaw
