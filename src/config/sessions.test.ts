...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ORCHID_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-orchid",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-orchid#general");
  });
  ...
  it("derives session transcripts dir from ORCHID_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ORCHID_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.orchid");
  ...
  it("resolves cross-agent paths when ORCHID_STATE_DIR differs from stored paths", () => {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.orchid"), () => {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
