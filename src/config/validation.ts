...
import type { OrchidConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OrchidConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OrchidConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OrchidSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OrchidConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OrchidConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as OrchidConfig,
  };
}
