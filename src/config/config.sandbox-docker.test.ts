...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("orchid-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "orchid-sandbox-browser-global" },
      agentBrowser: { network: "orchid-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("orchid-sandbox-browser-agent");
  });
