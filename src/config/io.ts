...
import type { OrchidConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareOrchidVersions } from "./version.js";
...
function coerceConfig(value: unknown): OrchidConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OrchidConfig;
}
...
function stampConfigVersion(cfg: OrchidConfig): OrchidConfig {
...
function warnIfConfigFromFuture(cfg: OrchidConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOrchidVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Orchid (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function normalizeExecSafeBinProfilesInConfig(cfg: OrchidConfig): void {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OrchidConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OrchidConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OrchidConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OrchidConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OrchidConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.ORCHID_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.ORCHID_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.ORCHID_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.ORCHID_WATCH_SESSION === "string" &&
        deps.env.ORCHID_WATCH_SESSION.trim().length > 0
          ? deps.env.ORCHID_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.ORCHID_WATCH_COMMAND === "string" &&
        deps.env.ORCHID_WATCH_COMMAND.trim().length > 0
          ? deps.env.ORCHID_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `ORCHID_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ORCHID_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ORCHID_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): OrchidConfig {
