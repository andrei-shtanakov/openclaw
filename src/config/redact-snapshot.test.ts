...
import type { ConfigFileSnapshot } from "./types.orchid.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.orchid/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/orchid/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/orchid/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/orchid/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/orchid/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = OrchidSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "OrchidConfig";
    const hints = mapSensitivePaths(OrchidSchema, "", {});
