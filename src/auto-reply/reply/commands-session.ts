...
import { scheduleGatewaySigusr1Restart, triggerOrchidRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Orchid in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerOrchidRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Orchid via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
