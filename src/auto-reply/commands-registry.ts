...
export function isCommandEnabled(cfg: OrchidConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OrchidConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OrchidConfig),
...
export function getCommandDetection(_cfg?: OrchidConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OrchidConfig) {
