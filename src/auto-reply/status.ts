...
type AgentDefaults = NonNullable<NonNullable<OrchidConfig["agents"]>["defaults"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.orchid/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
const formatVoiceModeLine = (config?: OrchidConfig, sessionEntry?: SessionEntry): string | null => {
  if (!config) {
    return null;
  }
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 Orchid ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: OrchidConfig): string {
  const lines = ["ℹ️ Help", ""];
