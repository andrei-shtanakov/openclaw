...
function summarizeGroupPolicy(cfg: OrchidConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: OrchidConfig): boolean {
...
function collectModels(cfg: OrchidConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: OrchidConfig): Set<string> {
  const baseCfg: OrchidConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: OrchidConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: OrchidConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: OrchidConfig): boolean {
...
function isBrowserEnabled(cfg: OrchidConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: OrchidConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: OrchidConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep ORCHID_STATE_DIR on a local-only volume and re-run "${formatCliCommand("orchid security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: OrchidConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer ORCHID_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const orchidGatewayToken =
    typeof env.ORCHID_GATEWAY_TOKEN === "string" && env.ORCHID_GATEWAY_TOKEN.trim()
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-orchid-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: OrchidConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: OrchidConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: orchid-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: OrchidConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: OrchidConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: OrchidConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: OrchidConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
