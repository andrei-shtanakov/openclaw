...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "orchid.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "orchid.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.ORCHID_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.ORCHID_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.ORCHID_NODE_OPTIONS_READY = "1";
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[orchid] ${parsed.error}`);
      process.exit(2);
    }
