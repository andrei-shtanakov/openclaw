...
function resolveSkillsLimits(config?: OrchidConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveOrchidMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: OrchidConfig }): {
...
export function buildWorkspaceSkillSnapshot(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`orchid skills check\` to audit.`
    : "";
