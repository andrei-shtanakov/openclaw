...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("orchid-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("orchid-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("orchid-sbx-");
