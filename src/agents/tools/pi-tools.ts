...
function resolveExecConfig(params: { cfg?: OrchidConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: OrchidConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createOrchidCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createOrchidReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
