...
const defaultTools = createOrchidCodingTools({ senderIsOwner: true });
...
describe("createOrchidCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createOrchidCodingTools({
      config,
    ...
    const anthropicTools = createOrchidCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createOrchidCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createOrchidCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
