...
describe("exec approvals", () => {
  ...
  beforeAll(async () => {
    ({ callGatewayTool } = await import("./gateway.js"));
  ...
  beforeEach(async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-test-"));
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-test-bin-"));
