...
type WebSearchConfig = NonNullable<OrchidConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OrchidConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("orchid configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.orchid.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://orchid.ai",
      "X-Title": "Orchid Web Search",
    },
