...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): OrchidConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as OrchidConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): OrchidConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as OrchidConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: OrchidConfig, accountId = "media") {
    await handleTelegramAction(
