...
import { createOrchidTools } from "../orchid-tools.js";
import { createHostSandboxFsBridge } from "../test-helpers/host-sandbox-fs-bridge.js";
import { createBrowserTool } from "./browser-tool.js";
import { __testing, createOrchidCodingTools } from "./pi-tools.js";
import { createOrchidReadTool, createSandboxedReadTool } from "./pi-tools.read.js";

const defaultTools = createOrchidCodingTools();
...
describe("createOrchidCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createOrchidCodingTools({ senderIsOwner: true }).find(
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOrchidCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-depth-policy-"));
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    ...
    const googleTools = createOrchidCodingTools({
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "orchid-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createOrchidReadTool(
      baseRead as unknown as Parameters<typeof createOrchidReadTool>[0],
    );
