...
function resolveDefaultModelRef(cfg?: OrchidConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: OrchidConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: OrchidConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureOrchidModelsJson(effectiveCfg, params.agentDir);
