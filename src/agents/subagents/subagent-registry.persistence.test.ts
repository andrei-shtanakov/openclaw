...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["ORCHID_STATE_DIR"]);
  let tempStateDir: string | null = null;

  const writePersistedRegistry = async (persisted: Record<string, unknown>) => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-subagent-"));
  ...
  it("uses isolated temp state when ORCHID_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "orchid-test-state"));
  });
});
