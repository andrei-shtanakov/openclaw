...
type CreateOrchidTools = (typeof import("./orchid-tools.js"))["createOrchidTools"];
export type CreateOrchidToolsOpts = Parameters<CreateOrchidTools>[0];
...
export async function getSessionsSpawnTool(opts: CreateOrchidToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createOrchidTools } = await import("./orchid-tools.js");
  const tool = createOrchidTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
