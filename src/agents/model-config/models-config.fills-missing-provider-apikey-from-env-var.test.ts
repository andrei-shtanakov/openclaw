...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureOrchidModelsJson(validated.config);

      const modelPath = path.join(resolveOrchidAgentDir(), "models.json");
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureOrchidModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
