...
describe("buildWorkspaceSkillsPrompt", () => {
  it("syncs merged skills into a target workspace", async () => {
    const sourceWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-"));
    const targetWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-"));
  ...
  it("filters skills based on env/config gates", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"orchid":{"requires":{"env":["GEMINI_API_KEY"]},"primaryEnv":"GEMINI_API_KEY"}}',
      body: "# Nano Banana\n",
    });
