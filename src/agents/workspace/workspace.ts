...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ORCHID_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".orchid", `workspace-${profile}`);
  }
  return path.join(home, ".orchid", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".orchid";
