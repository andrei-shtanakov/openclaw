...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ORCHID_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ORCHID_HOME: "/srv/orchid-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/orchid-home"), ".orchid", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".orchid", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("orchid-workspace-");
