...
const GOOGLE_LIVE_SINGLE_KEY = "ORCHID_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ORCHID_LIVE_ANTHROPIC_KEY",
    listVar: "ORCHID_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ORCHID_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ORCHID_LIVE_${base}_KEY`;
