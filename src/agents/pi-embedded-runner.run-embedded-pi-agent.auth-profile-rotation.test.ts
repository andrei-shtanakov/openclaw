...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureOrchidModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): OrchidConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-workspace-"));
    const now = Date.now();
