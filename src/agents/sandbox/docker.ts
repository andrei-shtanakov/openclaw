...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "orchid.sandbox=1");
  args.push("--label", `orchid.sessionKey=${params.scopeKey}`);
  args.push("--label", `orchid.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `orchid.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "orchid.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`orchid sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`orchid sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("orchid sandbox recreate --all");
}
