...
import { loadOrchidPlugins } from "../../plugins/loader.js";
...
describe("tool_result_persist hook", () => {
  ...
  it("loads tool_result_persist hooks without breaking persistence", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "orchid-toolpersist-"));
...
describe("before_message_write hook", () => {
  it("continues persistence when a before_message_write hook throws", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "orchid-before-write-"));
    process.env.ORCHID_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
