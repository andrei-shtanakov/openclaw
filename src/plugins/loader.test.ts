...
import { loadOrchidPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `orchid-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.ORCHID_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "orchid.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.ORCHID_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadOrchidPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.ORCHID_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.ORCHID_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadOrchidPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadOrchidPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOrchidPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.ORCHID_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ ORCHID_STATE_DIR: stateDir }, () => {
  ...
  it("returns a registry with error diagnostics when discovery throws", () => {
    process.env.ORCHID_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
    const spy = vi.spyOn(discoveryModule, "discoverOrchidPlugins").mockImplementation(() => {
