...
const loadOrchidPluginsMock = vi.fn();

vi.mock("./loader.js", () => ({
  loadOrchidPlugins: (params: unknown) => loadOrchidPluginsMock(params),
}));
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadOrchidPluginsMock.mockReturnValue(registry);
  return registry;
}

describe("resolvePluginTools optional tools", () => {
  beforeEach(() => {
    loadOrchidPluginsMock.mockClear();
  });
