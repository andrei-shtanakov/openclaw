...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `orchid-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        orchid: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function createZipperArchiveBuffer(): Promise<Buffer> {
  ...
  zip.file(
    ...
    JSON.stringify({
      name: "@orchid/zipper",
      version: "0.0.1",
      orchid: { extensions: ["./dist/index.js"] },
    }),
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.orchid/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without orchid.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@orchid/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("orchid.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@orchid/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@orchid/voice-call@0.0.1",
    });
