...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OrchidPluginDefinition;
  register?: OrchidPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OrchidPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OrchidPluginDefinition;
...
export function loadOrchidPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  let discovery: ReturnType<typeof discoverOrchidPlugins>;
  try {
    discovery = discoverOrchidPlugins({
      workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "orchid/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "orchid/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OrchidPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as OrchidPluginModule;
    } catch (err) {
