...
declare const __ORCHID_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "orchid";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["ORCHID_VERSION"],
      env["ORCHID_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Orchid version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __ORCHID_VERSION__ === "string" && __ORCHID_VERSION__) ||
  process.env.ORCHID_BUNDLED_VERSION ||
