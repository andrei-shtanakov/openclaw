...
import { discoverOrchidPlugins } from "../../plugins/discovery.js";
import type { OrchidPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, OrchidPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["ORCHID_PLUGIN_CATALOG_PATHS", "ORCHID_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<OrchidPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: OrchidPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: OrchidPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverOrchidPlugins({ workspaceDir: options.workspaceDir });
