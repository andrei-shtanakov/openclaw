...
function cfg(accounts?: Record<string, unknown> | null): OrchidConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as OrchidConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as OrchidConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as OrchidConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as OrchidConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as OrchidConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as OrchidConfig)).toBe("default");
    });
  });
});
