...
function telegramCfg(): OrchidConfig {
  return { channels: { telegram: { botToken: "tok" } } } as OrchidConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: OrchidConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as OrchidConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as OrchidConfig;
...
function createSignalAccountOverrideCfg(): OrchidConfig {
  return {
    channels: {
      signal: {
...
function createDiscordModerationOverrideCfg(params?: { channelsEnabled?: boolean }): OrchidConfig {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as OrchidConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as OrchidConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as OrchidConfig,
        accountId: undefined,
        params: {
