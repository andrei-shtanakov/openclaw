...
type EnvSnapshot = {
  HOME?: string;
  ORCHID_HOME?: string;
  ORCHID_STATE_DIR?: string;
  ORCHID_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    ORCHID_HOME: process.env.ORCHID_HOME,
    ORCHID_STATE_DIR: process.env.ORCHID_STATE_DIR,
    ORCHID_OAUTH_DIR: process.env.ORCHID_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: OrchidConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: OrchidConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "orchid-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.ORCHID_HOME = tempHome;
    process.env.ORCHID_STATE_DIR = path.join(tempHome, ".orchid");
    delete process.env.ORCHID_OAUTH_DIR;
    fs.mkdirSync(process.env.ORCHID_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when ORCHID_OAUTH_DIR is explicitly configured", async () => {
    process.env.ORCHID_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OrchidConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
