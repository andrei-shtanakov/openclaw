...
beforeAll(() => {
  envSnapshot = captureEnv(["ORCHID_PROFILE"]);
  process.env.ORCHID_PROFILE = "isolated";
});
...
vi.mock("../infra/orchid-root.js", () => ({
  resolveOrchidPackageRoot: vi.fn().mockResolvedValue("/tmp/orchid"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/orchid/pnpm-lock.yaml",
      markerPath: "/tmp/orchid/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("Orchid status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("orchid status --all") ||
          l.includes("orchid --profile isolated status --all") ||
          l.includes("orchid status --all") ||
          l.includes("orchid --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.ORCHID_GATEWAY_TOKEN = "abcd1234";
    try {
