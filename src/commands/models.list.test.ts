...
const ensureOrchidModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/orchid-agent/auth.json" });
const resolveOrchidAgentDir = vi.fn().mockReturnValue("/tmp/orchid-agent");
const ensureAuthProfileStore = vi.fn().mockReturnValue({ version: 1, profiles: {} });
const listProfilesForProvider = vi.fn().mockReturnValue([]);
const resolveAuthProfileDisplayLabel = vi.fn(({ profileId }: { profileId: string }) => profileId);
const resolveAuthStorePathForDisplay = vi
  .fn()
  .mockReturnValue("/tmp/orchid-agent/auth-profiles.json");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/orchid-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureOrchidModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/orchid-agent");
  });
