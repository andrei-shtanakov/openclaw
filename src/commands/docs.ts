...
const SEARCH_TOOL = "https://docs.orchid.ai/mcp.SearchOrchid";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.orchid.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('orchid docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.orchid.ai/");
      runtime.log(`Search: ${formatCliCommand('orchid docs "your query"')}`);
    }
    return;
  }
