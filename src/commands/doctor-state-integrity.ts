...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".orchid"].map((dir) => path.resolve(root, entry.name, dir));
...
function shouldRequireOAuthDir(cfg: OrchidConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.ORCHID_OAUTH_DIR?.trim()) {
    return true;
  }
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".orchid");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.orchid out of git; it contains credentials and session history.",
