...
function stripUnknownConfigKeys(config: OrchidConfig): {
  config: OrchidConfig;
  removed: string[];
} {
  const parsed = OrchidSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: OrchidConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: OrchidConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: OrchidConfig): Promise<{
  config: OrchidConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: OrchidConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: OrchidConfig): {
  config: OrchidConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: OrchidConfig): {
  config: OrchidConfig;
  changes: string[];
} {
...
function collectExecSafeBinScopes(cfg: OrchidConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: OrchidConfig): ExecSafeBinCoverageHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: OrchidConfig): {
  config: OrchidConfig;
  changes: string[];
  warnings: string[];
} {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".orchid");
  const targetPath = path.join(targetDir, "orchid.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".orchid", "orchid.json"),
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("orchid doctor --fix")}" to apply legacy migrations.`);
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("orchid doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("orchid doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("orchid doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("orchid doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("orchid doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "orchid doctor --fix" to remove these keys.');
    }
  }
