...
import type { OrchidConfig, GatewayBindMode } from "../config/config.js";
import { resolveGatewayAuth } from "../gateway/auth.js";
import { isLoopbackHost, resolveGatewayBindHost } from "../gateway/net.js";
import { resolveDmAllowState } from "../security/dm-policy-shared.js";
import { note } from "../terminal/note.js";

export async function noteSecurityWarnings(cfg: OrchidConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("orchid security audit --deep")}`;
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.orchid.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("orchid configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("orchid config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("orchid doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("orchid config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('orchid config set session.dmScope "per-channel-peer"') +
