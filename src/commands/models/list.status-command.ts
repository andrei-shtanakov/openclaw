...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOrchidAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("orchid models auth setup-token")}\` or \`${formatCliCommand("orchid configure")}\`.`
          : `Run \`${formatCliCommand("orchid configure")}\` or set an API key env var.`;
