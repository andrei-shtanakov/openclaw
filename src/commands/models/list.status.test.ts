...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOrchidAgentDir: vi.fn().mockReturnValue("/tmp/orchid-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/orchid-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/orchid-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOrchidAgentDir: mocks.resolveOrchidAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/orchid-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOrchidAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/orchid-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/orchid-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/orchid-agent-custom");
