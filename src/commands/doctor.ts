...
function resolveMode(cfg: OrchidConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveOrchidPackageRoot({
  ...
  let cfg: OrchidConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("orchid configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("orchid config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("orchid setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("orchid doctor --fix")}" to apply changes.`);
  }
