...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under ORCHID_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("orchid-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-oauth-sync-"));
    ...
    process.env.ORCHID_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-oauth-nosync-"));
    process.env.ORCHID_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside ORCHID_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-oauth-external-"));
    process.env.ORCHID_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* ORCHID_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ORCHID_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("orchid-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
