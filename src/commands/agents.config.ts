...
function resolveAgentName(cfg: OrchidConfig, agentId: string) {
...
function resolveAgentModel(cfg: OrchidConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OrchidConfig): AgentSummary[] {
