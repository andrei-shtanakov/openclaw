...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: OrchidConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: OrchidConfig) => { changed: boolean; next: OrchidConfig };
  defaultModel: string;
  overrideConfig: OrchidConfig;
  alreadyDefaultConfig: OrchidConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: OrchidConfig) => config,
      applyDefaultConfig: (config: OrchidConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: OrchidConfig = { agents: { defaults: {} } };
