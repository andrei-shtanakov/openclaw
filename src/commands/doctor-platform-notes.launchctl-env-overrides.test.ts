...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ORCHID_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ORCHID_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ORCHID_GATEWAY_TOKEN");
    expect(message).not.toContain("ORCHID_GATEWAY_PASSWORD");
  });
