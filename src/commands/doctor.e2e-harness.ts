...
export const resolveOrchidPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadOrchidPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveOrchidPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ORCHID_STATE_DIR;
  originalUpdateInProgress = process.env.ORCHID_UPDATE_IN_PROGRESS;
  process.env.ORCHID_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "orchid-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.ORCHID_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.ORCHID_UPDATE_IN_PROGRESS;
  } else {
    process.env.ORCHID_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
