...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Orchid status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("orchid security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("orchid security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.orchid.ai/faq");
  runtime.log("Troubleshooting: https://docs.orchid.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("orchid status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("orchid logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("orchid status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("orchid gateway probe")}`);
  }
}
