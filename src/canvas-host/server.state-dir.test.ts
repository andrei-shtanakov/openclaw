...
describe("canvas host state dir defaults", () => {
  it("uses ORCHID_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("orchid-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Orchid Canvas");
      } finally {
        await handler.close();
      }
    });
