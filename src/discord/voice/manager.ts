...
function resolveVoiceTtsConfig(params: { cfg: OrchidConfig; override?: TtsConfig }): {
  cfg: OrchidConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredOrchidTmpDir(), "discord-voice-"));
