...
const resolveStorePath = vi.fn(() => "/tmp/orchid-discord-process-test-sessions.json");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/orchid-discord-process-test-sessions.json");
  ...
  async function createBlockModeContext() {
    return await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/orchid-discord-process-test-sessions.json" },
        channels: {
          discord: {
