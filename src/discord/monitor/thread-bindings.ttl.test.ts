...
describe("thread binding ttl", () => {
  ...
  it("persists unbinds even when no manager is active", () => {
    const previousStateDir = process.env.ORCHID_STATE_DIR;
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "orchid-thread-bindings-"));
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.ORCHID_STATE_DIR = previousStateDir;
      }
