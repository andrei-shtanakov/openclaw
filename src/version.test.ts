...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "orchid-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "orchid", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "orchid", version: "2.3.4" });
  ...
  it("ignores non-orchid package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers ORCHID_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        ORCHID_VERSION: "9.9.9",
        ORCHID_SERVICE_VERSION: "2.2.2",
