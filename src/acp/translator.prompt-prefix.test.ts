...
describe("acp prompt cwd prefix", () => {
  async function runPromptWithCwd(cwd: string) {
    ...
    const previousOrchidHome = process.env.ORCHID_HOME;
    ...
    try {
      ...
      if (previousOrchidHome === undefined) {
        delete process.env.ORCHID_HOME;
      } else {
        process.env.ORCHID_HOME = previousOrchidHome;
      }
  ...
  it("redacts home directory in prompt prefix", async () => {
    const requestSpy = await runPromptWithCwd(path.join(os.homedir(), "orchid-test"));
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringMatching(/\[Working directory: ~[\\/]orchid-test\]/),
      }),
      { expectFinal: true },
    );
  });
  ...
  it("keeps backslash separators when cwd uses them", async () => {
    const requestSpy = await runPromptWithCwd(`${os.homedir()}\\orchid-test`);
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringContaining("[Working directory: ~\\orchid-test]"),
      }),
      { expectFinal: true },
    );
  });
