...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ORCHID_GATEWAY_PORT).toBe("18789");
    expect(env.ORCHID_GATEWAY_TOKEN).toBe("secret");
    expect(env.ORCHID_SERVICE_MARKER).toBe("orchid");
    expect(env.ORCHID_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ORCHID_SERVICE_VERSION).toBe("string");
    expect(env.ORCHID_SYSTEMD_UNIT).toBe("orchid-gateway.service");
    if (process.platform === "darwin") {
      expect(env.ORCHID_LAUNCHD_LABEL).toBe("ai.orchid.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ORCHID_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ORCHID_SYSTEMD_UNIT).toBe("orchid-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.ORCHID_LAUNCHD_LABEL).toBe("ai.orchid.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".orchid"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ORCHID_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".orchid-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ORCHID_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".orchid"));
  });

  it("uses ORCHID_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", ORCHID_STATE_DIR: "/var/lib/orchid" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/orchid"));
  });

  it("expands ~ in ORCHID_STATE_DIR", () => {
    const env = { HOME: "/Users/test", ORCHID_STATE_DIR: "~/orchid-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/orchid-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { ORCHID_STATE_DIR: "C:\\State\\orchid" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\orchid");
  });
});
