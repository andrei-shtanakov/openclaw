...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.ORCHID_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service ORCHID_GATEWAY_TOKEN does not match gateway.auth.token in orchid.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `orchid gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
