...
describe("resolveGatewayConnection", () => {
  ...
  it("uses ORCHID_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ ORCHID_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers ORCHID_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ ORCHID_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
