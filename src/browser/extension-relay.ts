...
const RELAY_AUTH_HEADER = "x-orchid-relay-token";
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/Orchid-Extension-Relay",
          revision: "0",
          userAgent: "Orchid-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "Orchid/extension-relay",
  ...
  try {
    ...
    if (
      ...
      (await probeAuthenticatedOrchidRelay({
