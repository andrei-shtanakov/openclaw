import type { BrowserConfig, BrowserProfileConfig, OrchidConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_ORCHID_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "orchid" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_ORCHID_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "orchid" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ORCHID_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ORCHID_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Orchid browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for orchid/orchid CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OrchidConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ORCHID_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "orchid";
