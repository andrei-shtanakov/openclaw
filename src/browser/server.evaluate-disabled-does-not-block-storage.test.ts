  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          orchid: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ORCHID_GATEWAY_PORT;
    process.env.ORCHID_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ORCHID_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ORCHID_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayToken === undefined) {
      ...
      process.env.ORCHID_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.ORCHID_GATEWAY_PASSWORD;
    } else {
      process.env.ORCHID_GATEWAY_PASSWORD = prevGatewayPassword;
    }
