...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".orchid-profile-decorated");
}
 ...
 */
export function decorateOrchidProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ORCHID_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ORCHID_BROWSER_COLOR).toUpperCase();
