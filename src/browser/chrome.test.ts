...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "orchid-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateOrchidProfile(userDataDir, { color: DEFAULT_ORCHID_BROWSER_COLOR });
    ...
    expect(def.name).toBe(DEFAULT_ORCHID_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_ORCHID_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(path.join(userDataDir, ".orchid-profile-decorated"), "utf-8");
    expect(marker.trim()).toMatch(/^\d+$/);
  });

  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateOrchidProfile(userDataDir, { color: "lobster-orange" });
    const localState = await readJson(path.join(userDataDir, "Local State"));
    const profile = localState.profile as Record<string, unknown>;
    const infoCache = profile.info_cache as Record<string, unknown>;
    const def = infoCache.Default as Record<string, unknown>;
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_ORCHID_BROWSER_PROFILE_NAME);
  });
});
...
describe("browser chrome helpers", () => {
  ...
  it("stopOrchidChrome no-ops when process is already killed", async () => {
    const proc = makeProc({ killed: true });
    await stopOrchidChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopOrchidChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopOrchidChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopOrchidChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
