...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.ORCHID_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/orchid" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "orchid",
        profiles: {
          orchid: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchOrchidChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.ORCHID_GATEWAY_PORT;
    process.env.ORCHID_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.ORCHID_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.ORCHID_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayToken === undefined) {
      ...
      process.env.ORCHID_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.ORCHID_GATEWAY_PASSWORD;
    } else {
      process.env.ORCHID_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
