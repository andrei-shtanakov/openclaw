...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredOrchidTmpDir: vi.fn(() => "/tmp/orchid"),
}));
vi.mock("../infra/tmp-orchid-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredOrchidTmpDir.mockReturnValue("/tmp/orchid");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredOrchidTmpDir.mockReturnValue("/tmp/orchid-preferred");
    const { res, outPath } = await waitForImplicitDownloadOutput({
      downloadUrl: "https://example.com/file.bin",
      suggestedFilename: "file.bin",
    });
    expect(typeof outPath).toBe("string");
    const expectedRootedDownloadsDir = path.join(path.sep, "tmp", "orchid-preferred", "downloads");
    const expectedDownloadsTail = `${path.join("tmp", "orchid-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredOrchidTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "orchid-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "orchid-preferred", "downloads")}${path.sep}`),
    );
  });
