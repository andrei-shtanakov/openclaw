...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ORCHID_GATEWAY_TOKEN"]);
    process.env.ORCHID_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
  });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-orchid-relay-token");
    expect(headers["x-orchid-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("accepts extension websocket access with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(`ws://127.0.0.1:${port}/extension`)["x-orchid-relay-token"];
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    const fakeRelay = createServer((req, res) => {
      if (req.url?.startsWith("/json/version")) {
        const header = req.headers["x-orchid-relay-token"];
        ...
        res.end(JSON.stringify({ Browser: "Orchid/extension-relay" }));
        return;
      }
  ...
  it("does not swallow EADDRINUSE when occupied port is not an orchid relay", async () => {
    const port = await getFreePort();
