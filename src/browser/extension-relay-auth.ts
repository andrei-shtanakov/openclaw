...
const RELAY_TOKEN_CONTEXT = "orchid-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const ORCHID_RELAY_BROWSER = "Orchid/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.ORCHID_GATEWAY_TOKEN?.trim() || process.env.ORCHID_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAuthTokenForPort(port: number): string {
  ...
  throw new Error(
    "extension relay requires gateway auth token (set gateway.auth.token or ORCHID_GATEWAY_TOKEN)",
  );
}

export async function probeAuthenticatedOrchidRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === ORCHID_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
